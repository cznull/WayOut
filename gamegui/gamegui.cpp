...
int solve(xort *a, xort *y, int n,std::set<int> &free) {
	int i, j, k;
	int r, c;
	xort temp;
	r = 0;
...
int spread(int x1, int y1, int x2, int  y2, int x, int y, int *map, xort *mat,int size) {
	...
	case 6:
		if (!mat[(y2*x + x2)*x*y + y1 * x + x1].v) {
			mat[(y2*x + x2)*x*y + y1 * x + x1].v = 1;
			if (x2 > 0) {
				spread(x1, y1, x2 - 1, y2, x, y, map, mat, size);
			}
			if (x2 < x - 1) {
				spread(x1, y1, x2 + 1, y2, x, y, map, mat, size);
			}
			if (y2 > 0) {
				spread(x1, y1, x2, y2 - 1, x, y, map, mat, size);
			}
			if (y2 < y - 1) {
				spread(x1, y1, x2, y2 + 1, x, y, map, mat, size);
			}
...
void draw(void) {
	...
	for (i = 0; i < size + 1; i++) {
		glVertex3f(0.0, i*(1.0 / size), 0.0);
		glVertex3f(1.0, i*(1.0 / size), 0.0);
		glVertex3f(i*(1.0 / size), 0.0, 0.0);
		glVertex3f(i*(1.0 / size), 1.0, 0.0);
	}
	glEnd();
	glBegin(GL_QUADS);
	for (j = 0; j < size; j++) {
		for (i = 0; i < size; i++) {
			if (i < msx&&j < msy) {
				glColor3f(state[j*size + i] ? 0.5 : 1.0, pr[j*size + i] ? 0.0 : 1.0, bu[j*size + i].v ? 0.0 : 1.0);
			}
			else {
				glColor3f(state[j*size + i] ? 0.3 : 0.5, pr[j*size + i] ? 0.0 : 0.5, bu[j*size + i].v ? 0.0 : 0.5);
			}
			float s, t;
			s = (map[j*size + i] % 4)*0.25;
			t = (map[j*size + i] / 4)*0.25;
			glTexCoord2f(s + 0.0f, t + 0.0f);
			glVertex3f(i*(1.0 / size), j*(1.0 / size), 0.0);
			glTexCoord2f(s + 0.25f, t + 0.0f);
			glVertex3f((i + 1)*(1.0 / size), j*(1.0 / size), 0.0);
			glTexCoord2f(s + 0.25f, t + 0.25f);
			glVertex3f((i + 1)*(1.0 / size), (j + 1)*(1.0 / size), 0.0);
			glTexCoord2f(s + 0.0f, t + 0.25f);
			glVertex3f(i*(1.0 / size), (j + 1)*(1.0 / size), 0.0);
		}
	}
	glEnd();
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE: {
		...
		pr = (int *)malloc(msx*msy * sizeof(int));
		bu = (xort *)malloc(msx*msy * sizeof(xort));
		bu1 = (xort *)malloc(msx*msy * sizeof(xort));
		bu0 = (xort *)malloc(msx*msy * sizeof(xort));
	...
	case WM_MOUSEMOVE: {
		...
		if (cx > cy) {
			mapx = (x - (cx - cy) / 2 - 0.05*cy) *  size / 0.9 / cy;
			mapy = (0.95*cy - y) *  size / 0.9 / cy;
		}
		else {
			mapx = (x - 0.05*cx) *  size / 0.9 / cx;
			mapy = (0.95*cy - (cx - cy) / 2 - y) *  size / 0.9 / cx;
		}
		if (MK_LBUTTON&wParam) {
			if (mapx != mapx1 || mapy != mapy1) {
				if (-1 < mapx&&mapx < size && -1 < mapy&&mapy < size) {
	...
	case WM_KEYDOWN: {
		...
		switch (wParam) {
		case 32: {
			int i, j, k, n, x, y, total, min1, min0;
			std::set<int> freev;
			std::set<int>::const_iterator it;
			std::vector<kandnb>::const_iterator itv;
			std::map<int,int>::const_iterator itm;
			std::vector<kandnb> hs, dc0, dc1;
			kandnb kn;
			x = msx;
			y = msy;
			n = msx * msy;
			...
			for (j = 0; j < y; j++) {
				for (i = 0; i < x; i++) {
					if (map[j*size + i] == 3) {
						if (state[j*size + i]) {
							kn.h = j * x + i;
							if (i > 0 && map[j*size + i - 1] != 0) {
								kn.nb.insert(j*x + i - 1);
							}
							if (i < x - 1 && map[j*size + i + 1] != 0) {
								kn.nb.insert(j*x + i + 1);
							}
							if (j > 0 && map[(j - 1)*size + i] != 0) {
								kn.nb.insert((j - 1)*x + i);
							}
							if (j < y - 1 && map[(j + 1)*size + i] != 0) {
								kn.nb.insert((j + 1)*x + i);
							}
							dc1.push_back(kn);
							kn.nb.clear();
			...
			if (dc1.size() > 0) {
				...
				for (itv = dc0.begin(); itv != dc0.end(); itv++) {
					bu[itv->h] += {1};
				}
				if (!solve(mat, bu, n, freev)) {
					int minv, mini, s;
					minv = n * 3 + 2;
					mini = -1;
					if (freev.size() < 20) {
						total = 1 << freev.size();
						for (i = 0; i < total; i++) {
							memcpy(bu1, bu, n * sizeof(xort));
							for (j = 0, it = freev.begin(); j < freev.size(); j++, it++) {
								if ((i >> j) % 2) {
									bu1[*it].v = 1;
									for (k = 0; k < n; k++) {
										bu1[k] += mat[k*n + *it];
							...
							while (t.size()) {
								int max = 0, maxp = 0;
								std::map<int, int> ct;
								for (itv = t.begin(); itv != t.end(); itv++) {
									for (it = itv->nb.begin(); it != itv->nb.end(); it++) {
										if (ct.find(*it) == ct.end()) {
											ct.insert(std::pair<int, int>(*it, 1));
										}
										else {
											ct[*it]++;
										}
								...
								for (itm = ct.begin(); itm != ct.end(); itm++) {
									if (itm->second > max) {
										max = itm->second;
										maxp = itm->first;
									}
								...
								for (j = 0; j < t.size(); j++) {
									if (t[j].nb.find(maxp) != t[j].nb.end()) {
										t.erase(t.begin() + j);
										j--;
									}
								}
			...
			if (dc0.size() > 0 || dc1.size() == 0) {
				...
				for (itv = dc1.begin(); itv != dc1.end(); itv++) {
					bu[itv->h] += {1};
				}
			...
			memset(pr, 0, size*size * sizeof(int));
			memset(bu, 0, size*size * sizeof(xort));
			if (min1 < min0) {
				for (i = msy - 1; i >= 0; i--) {
					for (j = msx - 1; j >= 0; j--) {
						bu[i*size + j] = bu1[i*msx + j];
					}
				}
				for (itv = dc1.begin(); itv != dc1.end(); itv++) {
					pr[(itv->h) / msx * size + (itv->h) % msx] = 1;
				}
			}
			else if (min0 < 2 * n + 3) {
				for (i = msy - 1; i >= 0; i--) {
					for (j = msx - 1; j >= 0; j--) {
						bu[i*size + j] = bu0[i*msx + j];
					}
				}
				for (itv = dc0.begin(); itv != dc0.end(); itv++) {
					pr[(itv->h) / msx * size + (itv->h) % msx] = 1;
